...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Fetch all root tiers for the project
    const rootTiers = await sql`
      SELECT id, name FROM tiers 
      WHERE project_id = ${id} AND parent_id IS NULL
      ORDER BY name
    `

    // Fetch all tiers and create worksheets
    for (const rootTier of rootTiers) {
      const tierHierarchy = await sql`
        WITH RECURSIVE tier_tree AS (
          SELECT id, name, parent_id, 0 as depth FROM tiers WHERE id = ${rootTier.id}
          UNION ALL
    ...
    return new Response(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="project-export-${Date.now()}.xlsx"`,
      },
    })
  } catch (error: any) {
    console.error("[v0] Project export error:", error)
